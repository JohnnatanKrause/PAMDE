...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const finalizarInspecaoBtn = document.getElementById('finalizarInspecaoBtn'); // Verifique no HTML se tem type="button"
    ...
    function validarCamposObrigatorios() {
        const camposParaValidar = [
            { input: marcaMedidaInput, nome: "Marca (Placa)" },
            { input: modeloMedidoInput, nome: "Modelo (Placa)" },
            { input: document.getElementById('potencia_placa_cv'), nome: "Potência de Placa (CV)" },
        ...
        for (const campo of camposParaValidar) {
            if (!campo.input) {
                console.warn(`[Validação] Input para '${campo.nome}' não encontrado no DOM. Pulando validação.`);
                continue;
            }
            ...
            if (inspecaoForm && inspecaoForm.elements) {
                for (let i = 0; i < inspecaoForm.elements.length; i++) {
                    ...
                    if (element.name && !element.name.endsWith('_status')) {
                        if ((element.type === 'text' || element.type === 'number' || element.type === 'textarea') && element.value.trim() !== "") { formModificado = true; break; }
                        if (element.tagName === 'SELECT' && element.value !== "" && element.selectedIndex !== 0) { formModificado = true; break; }
                        if (element.type === 'file' && element.files && element.files.length > 0) { formModificado = true; break; }
                    }
                }
            }
    ...
    function gerarPareceresEquipamento(equipamentoNominal, dadosColetados) {
        const pareceres = {};
        const toNumber = (val) => { const num = parseFloat(val); return isNaN(num) ? null : num; };
        const tensaoNominal = toNumber(equipamentoNominal.tensao_v_nominal);
        const tensoesMedidas = { f1: toNumber(dadosColetados.tensao_f1_medida), f2: toNumber(dadosColetados.tensao_f2_medida), f3: toNumber(dadosColetados.tensao_f3_medida) };
        const statusTensoes = { f1: dadosColetados.tensao_f1_medida_status, f2: dadosColetados.tensao_f2_medida_status, f3: dadosColetados.tensao_f3_medida_status };
        ['f1', 'f2', 'f3'].forEach(fase => {
            if (statusTensoes[fase] === 'NA') pareceres[`tensao_${fase}`] = "Não Aplicável";
            else if (statusTensoes[fase] === 'NM') pareceres[`tensao_${fase}`] = "Não Medido";
            else if (tensaoNominal === null || tensoesMedidas[fase] === null) pareceres[`tensao_${fase}`] = "Dados insuficientes";
            else {
                const limiteInferior = tensaoNominal * 0.95, limiteSuperior = tensaoNominal * 1.05;
                if (tensoesMedidas[fase] < limiteInferior) pareceres[`tensao_${fase}`] = `Não Conforme - Subtensão (${tensoesMedidas[fase]}V)`;
                else if (tensoesMedidas[fase] > limiteSuperior) pareceres[`tensao_${fase}`] = `Não Conforme - Sobretensão (${tensoesMedidas[fase]}V)`;
                else pareceres[`tensao_${fase}`] = `Conforme (${tensoesMedidas[fase]}V)`;
            }
        });
        const tensoesValidas = [tensoesMedidas.f1, tensoesMedidas.f2, tensoesMedidas.f3].filter(t => t !== null);
        if (tensoesValidas.length === 3 && !statusTensoes.f1 && !statusTensoes.f2 && !statusTensoes.f3) {
            const mediaTensoes = tensoesValidas.reduce((a, b) => a + b, 0) / 3;
            let maxDesvioTensao = 0; tensoesValidas.forEach(t => { maxDesvioTensao = Math.max(maxDesvioTensao, Math.abs(t - mediaTensoes)); });
            const percDesbTensao = (mediaTensoes > 0) ? (maxDesvioTensao / mediaTensoes) * 100 : 0;
            if (percDesbTensao > 3) pareceres.desbalanceamento_tensao = `Alerta - Desbalanceamento (${percDesbTensao.toFixed(1)}%)`;
            else pareceres.desbalanceamento_tensao = `Conforme (${percDesbTensao.toFixed(1)}%)`;
        } else pareceres.desbalanceamento_tensao = "Não foi possível calcular (dados faltantes/inválidos)";

        const tempMedida = toNumber(dadosColetados.temperatura_medida), tempStatus = dadosColetados.temperatura_medida_status;
        if (tempStatus === 'NA') pareceres.temperatura = "Não Aplicável";
        else if (tempStatus === 'NM') pareceres.temperatura = "Não Medido";
        else if (tempMedida === null) pareceres.temperatura = "Dados insuficientes";
        else if (tempMedida > 85) pareceres.temperatura = `Alerta - Temperatura Elevada (${tempMedida}°C)`;
        else pareceres.temperatura = `Conforme (${tempMedida}°C)`;

        const correnteNominal = toNumber(equipamentoNominal.corrente_a_nominal);
        const correntesMedidas = { f1: toNumber(dadosColetados.corrente_f1_medida), f2: toNumber(dadosColetados.corrente_f2_medida), f3: toNumber(dadosColetados.corrente_f3_medida) };
        const statusCorrentes = { f1: dadosColetados.corrente_f1_medida_status, f2: dadosColetados.corrente_f2_medida_status, f3: dadosColetados.corrente_f3_medida_status };
        ['f1', 'f2', 'f3'].forEach(fase => {
            if (statusCorrentes[fase] === 'NA') pareceres[`corrente_${fase}`] = "Não Aplicável";
            else if (statusCorrentes[fase] === 'NM') pareceres[`corrente_${fase}`] = "Não Medido";
            else if (correntesMedidas[fase] === null) pareceres[`corrente_${fase}`] = "Dados insuficientes";
            else if (correnteNominal === null) pareceres[`corrente_${fase}`] = `Registrado (${correntesMedidas[fase]}A) - Nominal N/D`;
            else if (correntesMedidas[fase] > (correnteNominal * 1.10)) pareceres[`corrente_${fase}`] = `Alerta - Corrente Alta (${correntesMedidas[fase]}A)`;
            else pareceres[`corrente_${fase}`] = `Registrado (${correntesMedidas[fase]}A)`;
        });
        ...
        if (correntesValidas.length === 3 && !statusCorrentes.f1 && !statusCorrentes.f2 && !statusCorrentes.f3) {
            ...
            if (mediaCorrentes > 0) {
                correntesValidas.forEach(c => { maxDesvioCorrente = Math.max(maxDesvioCorrente, Math.abs(c - mediaCorrentes)); });
                const percDesbCorrente = (maxDesvioCorrente / mediaCorrentes) * 100;
                if (percDesbCorrente > 10) pareceres.desbalanceamento_corrente = `Alerta - Desbalanceamento (${percDesbCorrente.toFixed(1)}%)`;
                else pareceres.desbalanceamento_corrente = `Conforme (${percDesbCorrente.toFixed(1)}%)`;
            } else if (mediaCorrentes === 0 && correntesValidas.every(c => c === 0)) pareceres.desbalanceamento_corrente = `Conforme (0.0%) - Sem Carga`;
            else pareceres.desbalanceamento_corrente = "Não foi possível calcular (dados zerados/inválidos)";
        } else pareceres.desbalanceamento_corrente = "Não foi possível calcular (dados faltantes/inválidos)";

        const potPlacaCV = toNumber(dadosColetados.potencia_placa_cv), potNominalCV = toNumber(equipamentoNominal.potencia_cv_nominal);
        if (potPlacaCV === null && potNominalCV === null) pareceres.potencia_placa = "Dados insuficientes";
        else if (potPlacaCV === null) pareceres.potencia_placa = `Nominal: ${potNominalCV} CV (Placa não informada)`;
        else if (potNominalCV === null || String(equipamentoNominal.potencia_cv_nominal).toUpperCase() === 'S/INF' || equipamentoNominal.potencia_cv_nominal === "") pareceres.potencia_placa = `Placa: ${potPlacaCV} CV (Nominal não cadastrado)`;
        else if (potPlacaCV === potNominalCV) pareceres.potencia_placa = "Conforme";
        else pareceres.potencia_placa = `Alerta - Divergência (Placa: ${potPlacaCV} CV, Nominal: ${potNominalCV} CV)`;

        const rotNominal = toNumber(equipamentoNominal.rotacao_rpm_nominal), rotMedida = toNumber(dadosColetados.rotacao_medida), rotStatus = dadosColetados.rotacao_medida_status;
        if (rotStatus === 'NA') pareceres.rotacao = "Não Aplicável";
        else if (rotStatus === 'NM') pareceres.rotacao = "Não Medido";
        else if (rotNominal === null || rotMedida === null) pareceres.rotacao = "Dados insuficientes";
        else {
            const limiteInferiorRot = rotNominal * 0.95, limiteSuperiorRot = rotNominal * 1.05;
            if (rotMedida < limiteInferiorRot) pareceres.rotacao = `Alerta - Rotação Baixa (${rotMedida} RPM)`;
            else if (rotMedida > limiteSuperiorRot) pareceres.rotacao = `Alerta - Rotação Alta (${rotMedida} RPM)`;
            else pareceres.rotacao = `Conforme (${rotMedida} RPM)`;
        }

        const rolDNominal = equipamentoNominal.rolamento_d_nominal, rolTNominal = equipamentoNominal.rolamento_t_nominal;
        const rolDMedido = dadosColetados.rolamento_d_medido, rolTMedido = dadosColetados.rolamento_t_medido;
        const rolDStatus = dadosColetados.rolamento_d_medido_status, rolTStatus = dadosColetados.rolamento_t_medido_status;
        ...
        const vibracaoStatus = dadosColetados.vibracao_status;
        ...
        const camposNominaisParaChecar = [
            { nominalKey: 'marca_nominal', placaKey: 'marca_medida' }, { nominalKey: 'modelo_nominal', placaKey: 'modelo_medido' },
            { nominalKey: 'potencia_cv_nominal', placaKey: 'potencia_placa_cv' }, { nominalKey: 'rolamento_d_nominal', placaKey: 'rolamento_d_medido' },
            { nominalKey: 'rolamento_t_nominal', placaKey: 'rolamento_t_medido' }
        ];
        let listaAtualizacoes = [];
        camposNominaisParaChecar.forEach(campo => {
            const valorNominal = equipamentoNominal[campo.nominalKey], valorPlaca = dadosColetados[campo.placaKey];
            if ((!valorNominal || String(valorNominal).toUpperCase() === 'S/INF' || String(valorNominal).trim() === "") && (valorPlaca && String(valorPlaca).trim() !== "")) {
    ...
    function formatarChaveParecer(chave) {
        ...
        return str.replace("Tensao F1", "Tensão F1").replace("Tensao F2", "Tensão F2").replace("Tensao F3", "Tensão F3")
            .replace("Corrente F1", "Corrente F1").replace("Corrente F2", "Corrente F2").replace("Corrente F3", "Corrente F3")
            .replace("Desbalanceamento Tensao", "Desbalanceamento de Tensão").replace("Desbalanceamento Corrente", "Desbalanceamento de Corrente")
            .replace("Ruido Rol Dianteiro", "Ruído Rol. Dianteiro").replace("Ruido Rol Traseiro", "Ruído Rol. Traseiro")
            .replace("Potencia Placa", "Potência (Placa vs Nominal)").replace("Rolamento Dianteiro", "Rol. Dianteiro (Placa vs Nominal)")
            .replace("Rolamento Traseiro", "Rol. Traseiro (Placa vs Nominal)");
    }
    function formatarMedicaoComStatus(valor, status) {
        if (status === 'NA') return 'N/A'; if (status === 'NM') return 'N/M';
    ...
    async function imageToBase64(url) {
        try {
            ...
            if (!response.ok) { console.error(`Falha ao buscar imagem: ${response.status} ${response.statusText} para ${url}`); return null; }
            const blob = await response.blob();
            return new Promise((resolve, reject) => {
                const reader = new FileReader();
                reader.onloadend = () => resolve(reader.result);
                reader.onerror = (error) => { console.error("Erro no FileReader para:", url, error); reject(null); };
    ...
    async function carregarLogosBase64() {
        try {
            if (!sessionStorage.getItem('logoOleBase64')) {
                ...
                if (logoOle) sessionStorage.setItem('logoOleBase64', logoOle); else console.warn("Logo Olé não pôde ser carregada.");
            }
            if (!sessionStorage.getItem('logoPandeBase64')) {
                const logoPande = await imageToBase64('assets/img/pande_logo.jpg');
                if (logoPande) sessionStorage.setItem('logoPandeBase64', logoPande); else console.warn("Logo PAMDE não pôde ser carregada.");
            }
    ...
    function attachEventListeners() {
        if (voltarDashboardBtn) {
            voltarDashboardBtn.addEventListener('click', () => {
                ...
                if (inspecoesRealizadas && !confirm("Você tem dados de inspeção não finalizados. Deseja realmente voltar e perder esses dados?")) return;
                window.location.href = 'dashboard.html';
            });
        }
